...
#include "bercik_cppGameModeBase.h"
#include "Math/UnrealMathUtility.h"

ACPP_bercik_cppGameMode::ACPP_bercik_cppGameMode()
{
	static ConstructorHelpers::FObjectFinder<UClass> BlueprintTilebercikSpawner(TEXT("Blueprint'/Game/Blueprints/BP_bercikSpawner.BP_bercikSpawner_C'"));
	if (BlueprintTilebercikSpawner.Object) {
		SubbercikSpawner = (UClass*)BlueprintTilebercikSpawner.Object;
	}
...
void ACPP_bercik_cppGameMode::BeginPlay()
{
	Super::BeginPlay();
}

void ACPP_bercik_cppGameMode::Spawn()
{
	...
	while (!mazeCheckTest)
	{
		...
		maze = ACPP_bercik_cppGameMode::GenMaze();
		mazeCheckTest = ACPP_bercik_cppGameMode::CheckMaze(maze);
	}
	//spawn maze 
	ACPP_bercik_cppGameMode::SpawnMaze(maze);
	//spawn player
	ACPP_bercik_cppGameMode::SpawnPlayer(maze);
}

TArray<FCharArray> ACPP_bercik_cppGameMode::GenMaze()
{
	FRandomStream rand;
...
void ACPP_bercik_cppGameMode::SpawnPlayer(TArray<FCharArray> maze)
{
	if (SubCharacter)
	{
...
void ACPP_bercik_cppGameMode::SpawnMaze(TArray<FCharArray> maze)
{
	FRandomStream rand;
	rand.GenerateNewSeed();
	int numberOfSpawnedberciks = 0;
	if (SubPassage && SubWall && SubbercikSpawner)
	{
		UWorld* world = GetWorld();
		if (world)
		{
			...
			for (int i = 0; i < 17; i++)
			{
				...
				for (int j = 0; j < 17; j++)
				{
					...
					{
						if (((i == 1 || i == 2) && ((j == 1 || j == 2) && numberOfSpawnedberciks == 0) || ((j == 15 || j == 16) && numberOfSpawnedberciks == 1)) || ((i == 15 || i == 16) && ((j == 1 || j == 2) && numberOfSpawnedberciks == 2) || ((j == 15 || j == 16) && numberOfSpawnedberciks == 3)))
						{
							++numberOfSpawnedberciks;
							world->SpawnActor<ABaseTile>(SubbercikSpawner, spawnLocation, rotator, spawnParams);
						}
						else
						{
...
bool ACPP_bercik_cppGameMode::CheckMaze(TArray<FCharArray> maze)
{
	for (int i = 0; i < 17; i++)
	{
